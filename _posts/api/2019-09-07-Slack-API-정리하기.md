---
layout: post
title:  "[API] Slack API 완벽 정리하기  "
date:   2019-09-07-00:46:00
author: 한만섭
categories: api
tags: slack slackAPI webhookAPI incoming webhookAPI chat postMessage
---
...
slack에는 다양한 API가 존재하는데 **incoming webhook API** 와 **Web API**에 대해서 정리해보려고 합니다.  API를 사용하기 전에 우선 APP을 만들어야합니다.  
...
sendMessage.addEventListener("click", async () => {
  const result = await axios({
    ...
    headers: {
      ...
      //
      Authorization:
        "Bearer xoxb-xxxxxxxx-xxxxxx-xxxxxxxxxxxx"
    }
  });
  console.log(result);
});
